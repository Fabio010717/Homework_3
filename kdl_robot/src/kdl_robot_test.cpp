...
// Global variables
std::vector<double> jnt_pos(7,0.0), jnt_vel(7,0.0), obj_pos(6,0.0),  obj_vel(6,0.0), aruco_pose(7,0.0),init_jnt_pos(7,0.0);
...
int main(int argc, char **argv)
{
    ...
    ros::Publisher cartesian_error_norm_pub = n.advertise<std_msgs::Float64>("/cartesian_error_norm", 1); 
    ...
    while ((ros::Time::now()-begin).toSec() < 2*traj_duration + init_time_slot)
    {
        if (robot_state_available)
        {
            ...
            //std::cout<<"EEpose.M "<<robot.getEEFrame().M<<std::endl;
            ...
            Eigen::Matrix<double,3,1> pos_error = computeLinearError(toEigen(des_pose.p), toEigen(robot.getEEFrame().p));
            Eigen::Matrix<double,3,1> orient_error = computeOrientationError(toEigen(des_pose.M), toEigen(robot.getEEFrame().M));
            Eigen::Matrix<double,6,1> x_tilde;
            x_tilde << pos_error, orient_error;
            double x_tilde_norm = x_tilde.norm();            
            
            //double x_tilde_norm = x_tilde.norm(); 
            tau = controller_.idCntr(des_pose, des_cart_vel, des_cart_acc, Kp, Ko, sqrt(Kp), sqrt(Ko));
            //std::cout << "tau: " << std::endl << tau << std::endl; 
            
            // Creation of ROS Float64 message
            std_msgs::Float64 cartesian_error_norm_msg;
            
            // Set errors
            cartesian_error_norm_msg.data = x_tilde_norm;
            
            // Publish errors
            cartesian_error_norm_pub.publish(cartesian_error_norm_msg);
            ...
            // Creation of a ROS Float64 message
            /*for(int i = 0; i < robot.getNrJnts(); i++)
            {
               std_msgs::Float64 joint_error_pos_norm_msg;
               joint_error_pos_norm_msg.data = pos_error(i, 0);
               
               // Publish the message
               joint_error_pos_norm_pub[i].publish(joint_error_pos_norm_msg);
            }*/
