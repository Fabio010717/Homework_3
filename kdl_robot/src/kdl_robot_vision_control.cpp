...
int main(int argc, char **argv)
{
    ...
    ros::Publisher sx_pub = n.advertise<std_msgs::Float64>("sx_component", 1);
    ros::Publisher sy_pub = n.advertise<std_msgs::Float64>("sy_component", 1);
    ros::Publisher sz_pub = n.advertise<std_msgs::Float64>("sz_component", 1);   
    ...
    while (ros::ok())
    {
        if (robot_state_available && aruco_pose_available)
        {
            ...
            double aruco_angle = std::acos(Eigen::Vector3d(0,0,1).dot(aruco_pos_n));
            KDL::Rotation Re = KDL::Rotation::Rot(KDL::Vector(r_o[0], r_o[1], r_o[2]), aruco_angle);
            KDL::Rotation R_offset = KDL::Rotation::Rot(KDL::Vector(0, 0, 1), -0.07);
            Eigen::Matrix<double,3,1> aruco_pos_base = toEigen(robot.getEEFrame().M*cam_T_object.p) + toEigen(robot.getEEFrame().p); // Aruco pos with respect to the base frame
            Eigen::Vector3d des_pos = Eigen::Vector3d(aruco_pos_base[0] / 2, aruco_pos_base[1], aruco_pos_base[2]); // the EE x coordinate is a half the aruco pos x coordinate*/
            
            // Debug //
            //std::cout<<"aruco_pos_n "<<aruco_pos_n<<std::endl;
            //std::cout<<"aruco_pos = "<<aruco_pos_n<<std::endl;
            //std::cout<<"aruco_pos_base = "<<aruco_pos_base<<std::endl;
            //std::cout<<"des_pos = "<<des_pos<<std::endl;
            //std::cout<<"R_offset = "<<R_offset<<std::endl;
            
            // compute errors
            /*Eigen::Matrix<double,3,1> e_o = computeOrientationError(toEigen(robot.getEEFrame().M*Re), toEigen(robot.getEEFrame().M));
            Eigen::Matrix<double,3,1> e_o_w = computeOrientationError(toEigen(Fi.M*R_offset), toEigen(robot.getEEFrame().M));
            Eigen::Matrix<double,3,1> e_p = computeLinearError(des_pos, toEigen(robot.getEEFrame().p));
            Eigen::Matrix<double,6,1> x_tilde;
            x_tilde << e_p,  e_o_w[0], e_o[1], e_o[2];*/
            
            // Debug //
            //std::cout<<"e_o"<<e_o_w[0]<<std::endl<<e_o[1]<<std::endl<<e_o[2]<<std::endl;
            //std::cout<<"Rd"<<toEigen(robot.getEEFrame().M*Re)<<std::endl;
            //std::cout<<"Re"<<toEigen(robot.getEEFrame().M)<<std::endl;
                        
            Eigen::Matrix<double,3,1> c_Po = toEigen(cam_T_object.p); //distance between the origins of the camera frame and the aruco one
            Eigen::Matrix<double,3,1> s = c_Po/c_Po.norm();
            Eigen::Matrix<double,3,3> Rc = toEigen(robot.getEEFrame().M);
            Eigen::Matrix<double,3,3> L_small = (-1/c_Po.norm())*(Eigen::Matrix<double,3,3>::Identity() - s*s.transpose());
            Eigen::Matrix<double,3,6> L = Eigen::Matrix<double,3,6>::Zero();
            Eigen::Matrix<double,6,6> R = Eigen::Matrix<double,6,6>::Zero();
           
            R.block(0,0,3,3) = Rc;
            R.block(3,3,3,3) = Rc;
            L.block(0,0,3,3) = L_small;
            ...
            dqd.data = k*L_J_pinv*sd + h*N*(qdi - toEigen(jnt_pos));
            ...
            // Creation of ROS Float64 messages
